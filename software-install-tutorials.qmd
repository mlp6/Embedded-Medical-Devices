...
::: callout-warning
Do not install the `cmake` extension pack because it includes `CMake Tools` that
has some incompatibilities with the Zephyr build tools.
:::
...
- Sign up for an account on the [Nordic Semiconductor DevAcademy](https://academy.nordicsemi.com/courses/nrf-connect-sdk-fundamentals/)

- Please complete the following tutorial to get your environment setup for using
the Nordic nRF Connect SDK: [Lesson 1 - nRF Connect SDK Introduction](https://academy.nordicsemi.com/lessons/lesson-1-nrf-connect-sdk-introduction/)
...
::: callout-tip
Be sure to use the `nRF Connect SDK v3.4.0 (LTS)` for this class.  The code
snippets provided throughout the semester will be based on this version of the
SDK.
:::
...
::: callout-warning
We will not have USB cables, memory keys and adapters available in the lab.
:::

- You will also need a USB memory key to transfer data from the lab
oscillopscopes to your laptop.  These oscilloscopes use a USB-A port, so if your
laptop only has USB-C ports, you will need an adaptor.

- Order a [MCP9808 High Accuracy I2C Temperature Sensor Breakout Board](https://www.adafruit.com/product/1782).  (Link is to Adafruit, but you
