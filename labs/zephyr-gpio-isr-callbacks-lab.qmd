...
be used to visualize output activity for the device.  While LEDs might seet chm
...
allow him to add your TAs and project labels to your repository.
...
1. Make sure that you can can build the Zephyr application "as is" after your clone the repository to your local machine.  
    ...
    - You should be using the `>=v3.4.0` SDK and toolchain.
...
- The example code uses `bool` variables to track events (e.g., button presses).
Replace these with `atomic` variables to ensure that the button press events are
not lost if the main loop is executing when the button is pressed.
...
each time it is pressed. 
- This incremental decrease in blink frequency should be defined by the
...
1. Create a UML state diagram of your firmware using Mermaid in `state_sequence_diagram.md`.  (Don't worry, we'll be populating the sequence diagram section soon when we get to threads.)
...
::: callout-note
You do not need to upload anything to Canvas.  You will be receiving feedback
through your GitLab repository.
:::
