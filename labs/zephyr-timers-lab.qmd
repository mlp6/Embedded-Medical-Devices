...
`Measure` functionality for `Period` and `Frequency` metrics).  You can
measurement the highest and lowest frequencies of your action LEDs (you don't
have to measure each discrete frequency).

- Demonstrate that your LEDs are perfectly out of phase with each other (i.e.,
when one is on, the other is off).  You can do this with a cellphone picture of
the oscilloscope screen showing both traces for the two action LEDs overlaid on
top of each other.
