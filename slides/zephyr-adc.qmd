...
<img src="images/adc/ADC-Block-Diagram.png" alt="ADC Block Diagram" width="800"/>
...
<img src="images/adc/mapping_analog_pins.png" alt="Mapping Analog Pins"/>

## How Digital Output is Calculated

<img src="images/adc/ADC-Calc-Dout.png" alt="Calculate Digital Output" width="800"/>

:fuelpump: **Note that the `RESULT` above is stored as 16-bit [2's complement](https://en.wikipedia.org/wiki/Two's_complement) values.** :fuelpump:
...
<img src="images/adc/reference_voltage_nrf52833.png" alt="Reference Voltage"/>
...
https://docs.zephyrproject.org/3.1.0/hardware/peripherals/adc.html#c.adc_sequence_callback
...
- The ADC sequence can be configured to perform multiple measurements ("extra samples") in a single sequence.  
...
- :warning: Be careful in how you calculate the buffer size for your array, depending on the scope of the array.
...
- The acquisition time for a single ADC sample is specified in the Devicetree:

```C
zephyr,acquisition-time = <ADC_ACQ_TIME(ADC_ACQ_TIME_MICROSECONDS, 40)>;  // 40 us is one of several valid sampling periods
```

<img src="images/adc/adc_acquisition_times.png" alt="ADC Acquisition Times" width="800"/>

- :warning: The acquisition time is not absolutely accurate and can dependent on many system factors, especially when dependent on the kernel counters.
...
- :warning: The logging module can also require non-trivial amounts of memory.  Immediate / deferred / optimized logging configurations can significantly impact the [stack usage](https://docs.zephyrproject.org/latest/services/logging/index.html#stack-usage).
