...
- `adc_read_dt()` is **blocking**, meaning that your kernel thread will wait for the ADC to complete before continuing.
...
- If you want to perform other tasks while the ADC is running, you can use the **asynchronous** ADC API.
  ...
  - [`adc_read_async_dt()`](https://docs.zephyrproject.org/latest/doxygen/html/group__adc__interface.html#ga009e3733b5b20eb6b26a201c9f9734fc) is the asynchronous version of `adc_read_dt()`, and similarly takes the `adc_dt_spec` struct directly.
- The **return value** of `adc_read_async_dt()` is immediate and indicates whether the
ADC sequence was **started** successfully — not whether sampling completed:
  - `0`: sequence started successfully
  - `-EINVAL`: invalid parameter value
  - `-ENOMEM`: buffer too small for requested samplings
  - `-ENOTSUP`: requested mode of operation not supported
  - `-EBUSY`: another sampling was triggered while the previous one was still in progress
...
- The signal must be initialized and passed to the `adc_read_async_dt()` function.
...
// adc_read_async_dt() takes the adc_dt_spec struct directly (no .dev member needed)
// Returns 0 if the sequence was started, or a negative error code on failure
...
::: callout-tip
What happens if you call `adc_read_async_dt()` again before the previous sequence is complete?
:::
