...
- Trying to read a button at a specific time is like only checking if you have
an incoming phone call at a specific time interval (i.e., no ring).

- Instead, think of the ringing phone as an event trigger that you allow to
happen "whenever".
...
- You might not be able to take a 2-hour phone call right when your phone rings,
but you can:

  - Acknowledge that you received the call (e.g., thumbs-up emoji text message)

  - Add an item to your "to do" list (i.e., "queue") to call the person back
  when you have time

- What you do when the phone rings (the interrupt) is captured in a **callback**
function.

## ISR -> Callback Function

- An interrupt can be used to call a **callback** function ("callback" = a
function executed in response to an interrupt or event).

- Need to execute the callback / ISR quickly to not paralyze the rest of
`main()` / other threads from running, otherwise device is paralyzed from
acting.
...
([gpio_remove_callback_dt()](https://docs.zephyrproject.org/latest/hardware/peripherals/gpio.html#c.gpio_remove_callback_dt))
or re-assigned from an ISR.
...
- In Zephyr, atomic operations can be performed using the `atomic` API, which provides functions for atomic read, write, and modify operations on variables.
...
- `main.c` - initialize GPIO struct, initialize callback struct, associate
callback with GPIO pin, define callback function, test for callback event state
in your code
...
stateDiagram-v2
    ...
    note right of prj_conf : This enables gpio.h to be included

    overlay : nrf52833dk_nrf52833.overlay
    overlay : Enable the GPIO ports/pins
    overlay : Create aliases

    state "main.c" as mainc {
        ...
        state "main()" as main {
            state "Configure port & pin" as config {
                direction LR
                [*] --> configA
                configA : Check GPIO port is ready
                configA --> configB
                configB : Configure/initialize GPIO pin
            }
            state "Associate GPIO pin with ISR/CB" as isrcb {
                direction LR
                [*] --> isrcbA
                isrcbA : Attach interrupt to pin
                isrcbA --> isrcbB
                isrcbB : Declare and initialize callback function
                isrcbB --> isrcbC
                isrcbC : Activate (add) the callback
            }
            config --> isrcb
        }
    }
...
static atomic_t sw0_event = ATOMIC_INIT(0);

// define callback function
void sw0_callback(const struct device *dev, struct gpio_callback *cb, uint32_t pins)
{
    // Set the event flag when the button is pressed
    atomic_set(&sw0_event, 1); // conditional statement in main() can now do something based on the 
                               // event detection we can also use actual system kernel event flags, 
...
- The contents of this function should consume minimal resources / time (i.e.,
cannot "block").

- Common action is to set an `event` or toggle the state of a Boolean, the value
of which is reset after action is taken in the main code.
...
if (!gpio_is_ready_dt(&sw0)) {
    ...
    return -ENODEV;  // exit code that will exit main()
}
...
if (err < 0) {
    LOG_ERR("Cannot configure sw0 pin [%d].", err);
    return err;
    // -ENOTSUP	if any of the configuration options is not supported
...
if (err < 0) {
    LOG_ERR("Cannot attach callback to sw0 [%d].", err);
}
...
- If you want a button to have multiple functionalities, you can do so by
changing the callback associated with the button.
...
- However, removing the callback like above will still cause the interrupt to
trigger. 

- Instead, it is best to remove the interrupt with
[gpio_pin_configure_dt](https://docs.zephyrproject.org/apidoc/latest/group__gpio__interface.html#ga24f0b4ad30e6a8c81f51a2813478c793):
...
- [Nordic DevAcademy: Reading buttons and controlling LEDs](https://academy.nordicsemi.com/courses/nrf-connect-sdk-fundamentals/lessons/lesson-2-reading-buttons-and-controlling-leds/)