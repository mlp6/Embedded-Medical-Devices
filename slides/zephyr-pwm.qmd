...
- Timing and duty cycle (pulse width) of `HIGH` and `LOW` signals can be used to
...
if (err) {
    LOG_ERR("Could not set pwm1 driver [%d].", err);
...
- The pulse length (duty cycle) can be changed "on the fly", but only changes at
the next period.
...
- Change the duty cycle of the PWM signal at a known timing interval.  Can be
done with a timer, callback function run at a set interval (e.g., ADC async),
etc.

- Need to consider how often to update based on frequency content of the desired
output signal.
...
## Lower-Level nRF PWM Control

[https://docs.nordicsemi.com/bundle/ps_nrf52833/page/pwm.html](https://docs.nordicsemi.com/bundle/ps_nrf52833/page/pwm.html)

![PWM Module](https://docs-be.nordicsemi.com/bundle/ps_nrf52833/page/ip/pwm/doc/image/pwm-module.svg?_LANG=enus)
...
- [Nordic Semiconductor: PinCtrl](https://docs.zephyrproject.org/latest/hardware/pinctrl/index.html)
